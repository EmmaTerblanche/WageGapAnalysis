...
Person2010 <- read.csv("/Users/mac/Desktop/Masters/Thesis Datawork/Data/GHS 2010 Data/zaf-statssa-ghs-2010-person-v2.2.csv")

# Create Data Set ----

Person2010WAP <- Person2010[ which(Person2010$Age > 14 & Person2010$Age < 65),]
## 60k+ observations

Person2010WAPE <- Person2010WAP[ which(Person2010WAP$Q21awge == "Yes"),]
## 20k+ observations

Person2010WAPES <- Person2010WAPE ##[ which(Person2010WAPE$Q22asto != "Unspecified" & Person2010WAPE$Q22asto > "0"),]
## 12k+ observations

# # New salary variable
# class(Person2010WAPES$Q22asto)
#
# Person2010WAPES$Q22asto <- as.numeric(Person2010WAPES$Q22asto)
#
# Person2010WAPES <- Person2010WAPES %>%
#     mutate(msal = ifelse(
#         Q22bsp == "Per week", Q22asto *4.2, ifelse(
#             Q22bsp == "Annually", Q22asto /12, Q22asto)))

# # Log salary variable ----
# Person2010WAPES$logsal <- log(Person2010WAPES$Q22asto)

# Age-squared -----
Person2010WAPES$AgeSq <- Person2010WAPES$Age ^2

# Education variable
Person2010WAPES <- mutate(Person2010WAPES, educnum = recode(Q16hiedu,
...
Person2010WAPES$educnum <- as.numeric(Person2010WAPES$educnum)

# Gender Variable

Person2010WAPES <- Person2010WAPES %>%
    mutate(Person2010WAPES, Gender1 = recode(Gender,
                                             "Male" = 0,
                                             "Female" = 1))

Person2010WAPES$Gender1 <- as.numeric(Person2010WAPES$Gender1)

# Province Variable
Person2010WAPES <- Person2010WAPES %>%
    mutate(Person2010WAPES, Prov1 = recode(Prov,
                                           "1" = "Western Cape",
                                           "2" = "Eastern Cape",
                                           "3" = "Northern Cape",
                                           "4" = "Free State",
...
#
# r1 <- lm(logsal ~ Race + educnum + AgeSq + Age + Gender1, data = Person2010WAPES)
# r1 %>%
#     summary()
#
# r2 <- lm(logsal ~ Race + bs(educnum, knots = c(7,12), degree = 1) + AgeSq + Age + Gender1, data = Person2010WAPES)
# r2 %>%
#     summary()
#
# Person2010WAPES %>% ggplot(aes(x = educnum, y = logsal)) + geom_smooth()
# # lyk anders as ander jare

# Create Subset ----
GHS2010 <- Person2010WAPES[,c("UqNr", "PersonNR", "Prov1", "Gender1", "Age", "person_wgt", "AgeSq", "Race", "educnum", "Q22asto", "Q22bsp", "Q23salc")]
...
class(GHS2010$TotSal)
class(GHS2010$Interval)
class(GHS2010$SalPeriod)
